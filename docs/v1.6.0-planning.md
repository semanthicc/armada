...
|-----------|----------------|--------------|-----------------|
| **Synonyms** ("login") | ✅ Found relevant code | ❌ None | Good |
| **Exact symbol** ("saveEmbeddingConfig") | ⚠️ Noisy (tests/docs) | ✅ Exact location | Poor for exact |
| **Technical** ("function that saves embedding config") | ✅ Found `updateProjectEmbeddingConfig` | N/A | **Excellent** |
| **Cross-ref** ("where is X used") | ⚠️ Misses some | ✅ Complete | Good enough |
...
|---------|----------------------|---------------|---------|
| Split mid-function | May return half function | Function = atomic chunk | 🟡 Moderate |
| Result attribution | "lines 45-89" | "function processPayment" | 🟢 UX benefit |
| Context continuity | 3-line overlap | No overlap needed | 🟢 Storage + |
| Docstring attachment | May be separate | Attached to function | 🟡 Quality + |
...
|---------|---------------|---------------|
| **Implementation** | ✅ Done (0 days) | ❌ ~13h work |
| **Dependencies** | None | tree-sitter (~3MB), grammars |
| **Language support** | ✅ Universal | ❌ TS/JS only initially |
| **Maintenance** | ✅ Minimal | ❌ Grammar updates |
...
### Priority 2: Improve Chunk Metadata 🔥
...
```typescript
expandQuery("login user") → ["login user", "authenticate", "sign in", "verify token"]
```
...
Instead:
1. ✅ Boost code over tests/docs (2h) - **BIGGEST IMPACT**
2. ✅ Add symbol-level metadata (4h) - **NO AST NEEDED**
3. ✅ Query expansion (3h) - **IMPROVES RECALL**
4. ❌ AST chunking (~13h) - **LOW ROI**
...
|---------|----------|--------|----------|
| Code result boosting | 2h | 🔥 HUGE | 1 |
| Symbol metadata extraction | 4h | 🔥 HIGH | 2 |
| Query expansion | 3h | 🟡 MEDIUM | 3 |
| Performance monitoring | 3h | 🟢 LOW | 4 |
| AST chunking | 13h | 🟢 LOW | 5 (DEFERRED) |

**Total for v1.6.0 MVP**: 9h
